...
import pro.haichuang.manni.transformer.CornerTransform;
    ...
    public  void LoadImage(String url, ImageView imageview) {
        if(mRoundCorners==-1){
            Glide.with(mContext)
                    ...
                    .apply(new RequestOptions().dontAnimate())//防止设置placeholder导致第一次不显示网络图片,只显示默认图片的问题
                    .apply(new RequestOptions().placeholder(defaultImg))
            ...
            Glide.with(mContext)
                    ...
                    .apply(new RequestOptions().dontAnimate())//防止设置placeholder导致第一次不显示网络图片,只显示默认图片的问题
                    .apply(new RequestOptions().placeholder(defaultImg))
                    .apply(new RequestOptions().transform(new CornerTransform(mContext, mRoundCorners)))
                    .apply(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.RESOURCE))
                    .into(imageview);
        }
    }
